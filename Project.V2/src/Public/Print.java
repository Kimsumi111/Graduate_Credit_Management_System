...
public class Print
{
	...
	public void Option(int n)
	{
		...
		switch (n)
		{
			...
			case 2:
				...
				System.out.println("2번 : 강의 정보를 수정합니다.");
	...
	public int User(User User, int n,String Str)
	{
		...
		if (n != 5)
		{
			for(int i = 0; i<100; i++)
			{
				if(User.getInt("Level",i) == n && User.getString("Term",i).equals(Str))
				{
					...
					sum1 = sum1 + (User.getDouble("Credit",i)*User.getDouble("Grade",i));
					avg = avg + User.getDouble("Credit",i);
				}
			}
		}
		else
			for(int i = 4; i<20; i++)
			{
				...
				for(int j = 0; j<100; j++)
				{
					if(User.getInt("Level",j) == IntMem[0] && User.getString("Term",j).equals(Requ.Term[IntMem[1]]))
					{
						...
						sum1 = sum1 + (User.getDouble("Credit",j)*User.getDouble("Grade",j));
						avg = avg + User.getDouble("Credit",j);
					}
				}
		...
		{
			System.out.print("총 이수 학점 : " + sum);
			System.out.println("| 평균 성적 : " + (sum1/avg));
			return -1;
		}
	}
